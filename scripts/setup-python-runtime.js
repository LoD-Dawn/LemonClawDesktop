 ...
 * - Supports local offline archive via DICLAW_PORTABLE_PYTHON_ARCHIVE
 * - Supports optional mirror URL via DICLAW_PORTABLE_PYTHON_URL
...
const DEFAULT_WINDOWS_EMBED_PYTHON_VERSION = process.env.DICLAW_WINDOWS_EMBED_PYTHON_VERSION || '3.11.9';
const DEFAULT_WINDOWS_EMBED_PYTHON_ZIP = `python-${DEFAULT_WINDOWS_EMBED_PYTHON_VERSION}-embed-amd64.zip`;
const DEFAULT_WINDOWS_EMBED_PYTHON_URL = process.env.DICLAW_WINDOWS_EMBED_PYTHON_URL
  || `https://www.python.org/ftp/python/${DEFAULT_WINDOWS_EMBED_PYTHON_VERSION}/${DEFAULT_WINDOWS_EMBED_PYTHON_ZIP}`;
const DEFAULT_GET_PIP_URL = process.env.DICLAW_WINDOWS_GET_PIP_URL || 'https://bootstrap.pypa.io/get-pip.py';
const DEFAULT_PIP_PYZ_URL = process.env.DICLAW_WINDOWS_PIP_PYZ_URL || 'https://bootstrap.pypa.io/pip/pip.pyz';
    ...
    try {
      ...
      if (required) {
        throw new Error(
          ...
          + 'Set DICLAW_WINDOWS_PIP_PYZ_URL to a reachable mirror if needed. '
...
async function resolveArchive(required) {
  const envArchive = resolveInputPath(process.env.DICLAW_PORTABLE_PYTHON_ARCHIVE);
  if (envArchive) {
    if (!isNonEmptyFile(envArchive)) {
      throw new Error(`DICLAW_PORTABLE_PYTHON_ARCHIVE points to an invalid file: ${envArchive}`);
    }
    console.log(`[setup-python-runtime] Using local archive from DICLAW_PORTABLE_PYTHON_ARCHIVE: ${envArchive}`);
  ...
  const urlFromEnv = typeof process.env.DICLAW_PORTABLE_PYTHON_URL === 'string'
    ? process.env.DICLAW_PORTABLE_PYTHON_URL.trim()
    : '';
  ...
  if (!downloadUrl) {
    if (required) {
      throw new Error(
        ...
        + 'Set DICLAW_PORTABLE_PYTHON_ARCHIVE to a local package or '
        + 'DICLAW_PORTABLE_PYTHON_URL to a downloadable runtime archive URL.'
      );
    }
  ...
  try {
    ...
    if (required) {
      throw new Error(
        ...
        + 'Set DICLAW_PORTABLE_PYTHON_ARCHIVE to a local offline package or '
        + 'set DICLAW_PORTABLE_PYTHON_URL to a reachable mirror. '
        + `Original error: ${error instanceof Error ? error.message : String(error)}`
      );
    }
...
async function ensurePortablePythonRuntime(options = {}) {
  ...
  const shouldRun = process.platform === 'win32'
    || required
    || process.env.DICLAW_SETUP_PYTHON_RUNTIME_FORCE === '1';
