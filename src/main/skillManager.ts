...
function buildSkillEnv(): Record<string, string | undefined> {
  ...
  env.DICLAW_ELECTRON_PATH = getElectronNodeRuntimePath();
...
const readZipResponseBuffer = async (
  ...
  if (maxBytes && Number.isFinite(contentLength) && contentLength > maxBytes) {
    throw new Error(`Skill package exceeds limit of ${Math.floor(maxBytes / 1024 / 1024)} MB`);
  }
  ...
  async downloadSkill(
    ...
    try {
      ...
      if (fs.existsSync(localSource)) {
        ...
        const tempRoot = fs.mkdtempSync(path.join(app.getPath('temp'), 'diclaw-skill-'));
        cleanupPath = tempRoot;
