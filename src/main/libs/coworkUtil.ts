...
export function ensureElectronNodeShim(electronPath: string, npmBinDir?: string): string | null {
  try {
    ...
    const nodeShContent = [
      '#!/usr/bin/env bash',
      'if [ -z "${DICLAW_ELECTRON_PATH:-}" ]; then',
      '  echo "DICLAW_ELECTRON_PATH is not set" >&2',
      '  exit 127',
      'fi',
      'exec env ELECTRON_RUN_AS_NODE=1 "${DICLAW_ELECTRON_PATH}" "$@"',
      '',
    ].join('\n');
    ...
    if (process.platform === 'win32') {
      ...
      const nodeCmdContent = [
        '@echo off',
        'if "%DICLAW_ELECTRON_PATH%"=="" (',
        '  echo DICLAW_ELECTRON_PATH is not set 1>&2',
        '  exit /b 127',
        ')',
        'set ELECTRON_RUN_AS_NODE=1',
        '"%DICLAW_ELECTRON_PATH%" %*',
        '',
      ].join('\r\n');
    ...
    if (npmBinDir && existsSync(npmBinDir)) {
      ...
      if (existsSync(npxCliJs)) {
        ...
        // npx.cmd for Windows — uses %DICLAW_NPM_BIN_DIR% env var to avoid
        // hardcoding paths that may contain non-ASCII chars (breaks GBK cmd.exe).
        if (process.platform === 'win32') {
          const npxCmd = join(shimDir, 'npx.cmd');
          const npxCmdContent = [
            '@echo off',
            '"%~dp0node.cmd" "%DICLAW_NPM_BIN_DIR%\\npx-cli.js" %*',
            '',
          ].join('\r\n');
          writeFileSync(npxCmd, npxCmdContent, 'utf8');
          coworkLog('INFO', 'resolveNodeShim', `Created npx.cmd shim: ${npxCmd} (using env var DICLAW_NPM_BIN_DIR)`);
        }
      } else {
      ...
      if (existsSync(npmCliJs)) {
        ...
        // npm.cmd for Windows — uses %DICLAW_NPM_BIN_DIR% env var to avoid
        // hardcoding paths that may contain non-ASCII chars (breaks GBK cmd.exe).
        if (process.platform === 'win32') {
          ...
          coworkLog('INFO', 'resolveNodeShim', `Created npm.cmd shim: ${npmCmd} (using env var DICLAW_NPM_BIN_DIR)`);
        }
      } else {
...
function resolveWindowsGitBashPath(): string | null {
  ...
  // 2. Bundled PortableGit (preferred default in DiClaw package)
...
function ensureWindowsBashUtf8InitScript(): string | null {
  try {
    ...
    const content = [
      '#!/usr/bin/env bash',
      '# Auto-generated by DiClaw – switch Windows console code page to UTF-8',
...
function applyPackagedEnvOverrides(env: Record<string, string | undefined>): void {
  ...
  if (app.isPackaged && !env.DICLAW_ELECTRON_PATH) {
    env.DICLAW_ELECTRON_PATH = electronNodeRuntimePath;
  }
  ...
  if (process.platform === 'win32') {
    ...
    if (configuredBashPath && bashPath === configuredBashPath) {
      ...
      if (!configuredHealth.ok) {
        ...
        if (fallbackPath && fallbackPath !== configuredBashPath) {
          ...
          env.DICLAW_GIT_BASH_RESOLUTION_ERROR = diagnostic;
          coworkLog('WARN', 'resolveGitBash', diagnostic);
          bashPath = null;
        }
      }
    ...
    if (bashPath) {
      ...
      delete env.DICLAW_GIT_BASH_RESOLUTION_ERROR;
      ...
      env.DICLAW_GIT_BASH_RESOLUTION_ERROR = truncateDiagnostic(diagnostic);
    }
  ...
  env.DICLAW_NPM_BIN_DIR = npmBinDir;
  ...
  if (shouldInjectShim) {
    ...
    delete env.DICLAW_NODE_SHIM_ACTIVE;
...
function verifyNodeEnvironment(env: Record<string, string | undefined>): void {
        ...
        if (tool === 'node' && resolvedForExec) {
          try {
            ...
            if (process.platform === 'win32' && /\.cmd$/i.test(resolvedForExec)) {
              execTarget = env.DICLAW_ELECTRON_PATH || process.execPath;
            }
  ...
  coworkLog('INFO', tag, `DICLAW_ELECTRON_PATH=${env.DICLAW_ELECTRON_PATH || '(not set)'}`);
  coworkLog('INFO', tag, `DICLAW_NPM_BIN_DIR=${env.DICLAW_NPM_BIN_DIR || '(not set)'}`);
...
export function getSkillsRoot(): string {
  ...
  const envRoots = [process.env.DICLAW_SKILLS_ROOT, process.env.SKILLS_ROOT]
...
export async function getEnhancedEnv(target: OpenAICompatProxyTarget = 'local'): Promise<Record<string, string | undefined>> {
  ...
  env.DICLAW_SKILLS_ROOT = skillsRoot; // Alternative name for clarity
  if (process.platform === 'win32' || env.DICLAW_NODE_SHIM_ACTIVE === '1') {
    env.DICLAW_ELECTRON_PATH = getElectronNodeRuntimePath().replace(/\\/g, '/');
  } else {
    delete env.DICLAW_ELECTRON_PATH;
  }
  ...
  if (internalApiBaseURL) {
    env.DICLAW_API_BASE_URL = internalApiBaseURL;
  }
