...
export class OpenClawEngineManager extends EventEmitter {
  ...
  private resolveRuntimeMetadata(): RuntimeMetadata {
    ...
    const envCandidates = envRuntimeDir
      ? [
          path.resolve(envRuntimeDir),
          path.join(path.resolve(envRuntimeDir), 'current'),
        ]
      : [];
    const devBaseDirs = Array.from(new Set([
      process.cwd(),
      app.getAppPath(),
      path.dirname(app.getAppPath()),
      path.resolve(app.getAppPath(), '..', '..'),
    ]));
    const devCandidates = devBaseDirs.map((baseDir) =>
      path.join(baseDir, 'vendor', 'openclaw-runtime', 'current'),
    );
    ...
    const expectedPathHint = app.isPackaged
      ? path.join(process.resourcesPath, 'cfmind')
      : envRuntimeDir || path.join(process.cwd(), 'vendor', 'openclaw-runtime', 'current');

    if (!runtimeRoot) {
      console.warn('[OpenClaw] runtime not found. searched:', candidateRoots);
      return {
        root: null,
        version: null,
        expectedPathHint,
