...
function buildReminderTaskPrompt(body: string): string {
  const eventText = buildSystemEventText(body);
  return [
    eventText,
    '请直接输出这条提醒内容，不要声称之后会再提醒，也不要执行额外工具。',
  ].join('\n');
}
...
function buildLocalTimeContextPrompt(now: Date): string {
  return [
    `Current local datetime: ${toLocalIsoWithOffset(now)}`,
    `Current local time: ${formatLocalClock(now)}`,
    `Current UTC offset: ${formatUtcOffset(now)}`,
    'Interpret ambiguous times using this local timezone.',
  ].join('\n');
}
...
export function normalizeDetectedScheduledTaskRequest(
  ...
  return {
    ...
    taskName,
    payloadText,
    taskPrompt: buildReminderTaskPrompt(reminderBody),
...
function buildScheduledTaskDetectionPrompt(now: Date): string {
  return [
    ...
    buildLocalTimeContextPrompt(now),
