...
const CodeBlock: React.FC<any> = ({ node, className, children, ...props }) => {
  ...
  if (!isInline) {
    ...
    return (
      <div className="my-3 rounded-xl overflow-hidden border dark:border-dark-border border-border relative shadow-subtle">
        <div className="dark:bg-dark-surface-muted bg-surface-muted px-4 py-2 text-xs dark:text-dark-text-secondary text-text-secondary font-medium flex items-center justify-between">
          <span>{match[1]}</span>
          <button
            type="button"
            onClick={handleCopy}
            className="p-1.5 rounded-md dark:hover:bg-dark-surface-hover hover:bg-surface-hover transition-colors"
            title={i18nService.t('copyToClipboard')}
            aria-label={i18nService.t('copyToClipboard')}
          >
            {isCopied ? (
  ...
  const inlineClassName = [
    'inline bg-transparent px-0.5 text-[0.92em] font-mono font-medium dark:text-dark-text text-text-primary',
    normalizedClassName,
  ].filter(Boolean).join(' ');
...
const createMarkdownComponents = (
  ...
  p: ({ node, className, children, ...props }: any) => (
    <p className="my-1 first:mt-0 last:mb-0 leading-6 dark:text-dark-text text-text-primary" {...props}>
      {children}
    </p>
  ),
  strong: ({ node, className, children, ...props }: any) => (
    <strong className="font-semibold dark:text-dark-text text-text-primary" {...props}>
      {children}
    </strong>
  ),
  h1: ({ node, className, children, ...props }: any) => (
    <h1 className="text-2xl font-semibold mt-6 mb-3 dark:text-dark-text text-text-primary" {...props}>
      {children}
    </h1>
  ),
  h2: ({ node, className, children, ...props }: any) => (
    <h2 className="text-xl font-semibold mt-5 mb-2 dark:text-dark-text text-text-primary" {...props}>
      {children}
    </h2>
  ),
  h3: ({ node, className, children, ...props }: any) => (
    <h3 className="text-lg font-semibold mt-4 mb-2 dark:text-dark-text text-text-primary" {...props}>
      {children}
    </h3>
  ),
  ul: ({ node, className, children, ...props }: any) => (
    <ul className="list-disc pl-5 my-1.5 dark:text-dark-text text-text-primary" {...props}>
      {children}
    </ul>
  ),
  ol: ({ node, className, children, ...props }: any) => (
    <ol className="list-decimal pl-6 my-1.5 dark:text-dark-text text-text-primary" {...props}>
      {children}
    </ol>
  ),
  li: ({ node, className, children, ...props }: any) => (
    <li className="my-0.5 leading-6 dark:text-dark-text text-text-primary" {...props}>
      {children}
    </li>
  ),
  ...
  table: ({ node, className, children, ...props }: any) => (
    <div className="my-4 overflow-x-auto rounded-xl border dark:border-dark-border border-border">
  ...
  thead: ({ node, className, children, ...props }: any) => (
    <thead className="dark:bg-dark-surface bg-surface-hover" {...props}>
      {children}
    </thead>
  ),
  tbody: ({ node, className, children, ...props }: any) => (
    <tbody className="divide-y dark:divide-dark-border divide-border" {...props}>
      {children}
    </tbody>
  ),
  tr: ({ node, className, children, ...props }: any) => (
    <tr className="divide-x dark:divide-dark-border divide-border" {...props}>
      {children}
    </tr>
  ),
  th: ({ node, className, children, ...props }: any) => (
    <th className="px-4 py-2 text-left font-semibold dark:text-dark-text text-text-primary" {...props}>
      {children}
    </th>
  ),
  td: ({ node, className, children, ...props }: any) => (
    <td className="px-4 py-2 dark:text-dark-text text-text-primary" {...props}>
      {children}
    </td>
  ),
  ...
  hr: ({ node, ...props }: any) => (
    <hr className="my-5 dark:border-dark-border border-border" {...props} />
  ),
