...
const TaskListItem: React.FC<TaskListItemProps> = ({ task, onRequestDelete }) => {
  ...
  return (
    <div
      className="grid grid-cols-[1.2fr_1fr_110px_40px] items-center gap-3 px-4 py-3 border-b dark:border-dark-border/50 border-border/50 hover:bg-surface-hover/50 dark:hover:bg-dark-surface-hover/50 cursor-pointer transition-colors"
    ...
    >
      <div className="min-w-0">
        ...
        {task.description && (
          <div className="text-xs truncate dark:text-dark-text-secondary text-text-secondary">
            {task.description}
          </div>
        )}
      </div>
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <button
          ...
          className={`relative shrink-0 w-7 h-4 rounded-full transition-colors ${
            task.enabled ? 'bg-primary' : 'dark:bg-dark-surface-hover bg-border'
          }`}
        >
          <span
...
const TaskList: React.FC<TaskListProps> = ({ onRequestDelete }) => {
  ...
  return (
    <div>
      <div className="grid grid-cols-[1.2fr_1fr_110px_40px] items-center gap-3 px-4 py-2 border-b dark:border-dark-border/50 border-border/50">
