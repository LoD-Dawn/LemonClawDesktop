...
import { Skill, MarketplacePagination, MarketplaceSkill, MarketTag } from '../../types/skill';
...
const SkillsManager: React.FC = () => {
  ...
  const [marketplacePagination, setMarketplacePagination] = useState<MarketplacePagination>(DEFAULT_MARKETPLACE_PAGINATION);
  ...
  useEffect(() => {
    ...
    skillService.fetchMarketplaceSkills({
      page: marketplacePagination.page,
      ...
      if (data.error) {
        setSkillActionError(data.error);
      }
      setIsLoadingMarketplace(false);
    });
    return () => { isActive = false; };
  }, [marketplacePagination.page, marketplacePagination.pageSize]);
  ...
  const filteredMarketplaceSkills = useMemo(() => {
    ...
    if (activeMarketTag !== 'all') {
      results = results.filter(skill => skill.tagIds?.includes(activeMarketTag));
    }
    return results;
  ...
  const handleInstallMarketplaceSkill = async (skill: MarketplaceSkill) => {
    if (installingSkillId || !skill.url || skill.permission?.canUse === false) return;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 text-sm rounded-xl dark:bg-dark-surface bg-surface dark:text-dark-text text-text-primary dark:placeholder-dark-text-secondary placeholder-text-secondary border dark:border-dark-border border-border focus:outline-none focus:ring-2 focus:ring-primary"
          />
        </div>
          ...
          {isAddSkillMenuOpen && (
            ...
            >
              <button
                ...
                className="w-full flex items-center gap-3 px-3 py-2.5 text-sm dark:text-dark-text text-text-primary dark:hover:bg-dark-surface-hover hover:bg-surface-hover transition-colors disabled:opacity-50"
              >
                <UploadIcon className="h-4 w-4 dark:text-dark-text-secondary text-text-secondary" />
              ...
              <button
                ...
                className="w-full flex items-center gap-3 px-3 py-2.5 text-sm dark:text-dark-text text-text-primary dark:hover:bg-dark-surface-hover hover:bg-surface-hover transition-colors disabled:opacity-50"
              >
                <FolderOpenIcon className="h-4 w-4 dark:text-dark-text-secondary text-text-secondary" />
                <span>{i18nService.t('uploadSkillFolder')}</span>
              </button>
              <button
                type="button"
                onClick={handleOpenGithubImport}
                className="w-full flex items-center gap-3 px-3 py-2.5 text-sm dark:text-dark-text text-text-primary dark:hover:bg-dark-surface-hover hover:bg-surface-hover transition-colors"
              >
                <LinkIcon className="h-4 w-4 dark:text-dark-text-secondary text-text-secondary" />
      ...
      <div className="flex items-center border-b dark:border-dark-border border-border">
        <button
          type="button"
      ...
      <div className="grid grid-cols-2 gap-3">
        {filteredSkills.length === 0 ? (
          <div className="col-span-2 text-center py-8 text-sm dark:text-dark-text-secondary text-text-secondary">
            ...
            >
              <div className="flex items-start justify-between mb-2">
                <div className="flex items-center gap-2 min-w-0">
                  <div className="w-7 h-7 rounded-lg dark:bg-dark-surface bg-surface flex items-center justify-center flex-shrink-0">
                    <PuzzleIcon className="h-4 w-4 dark:text-dark-text-secondary text-text-secondary" />
                  </div>
                  <span className="text-sm font-medium dark:text-dark-text text-text-primary truncate">
                    {skill.name}
                  </span>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <div
                    className={`w-9 h-5 rounded-full flex items-center transition-colors cursor-pointer flex-shrink-0 ${
                      skill.enabled ? 'bg-primary' : 'dark:bg-dark-border bg-border'
                    }`}
              ...
              <p className="text-xs dark:text-dark-text-secondary text-text-secondary line-clamp-2 mb-2">
                {skillService.getLocalizedSkillDescription(skill.id, skill.name, skill.description)}
              </p>

              <div className="flex items-center gap-2 text-[10px] dark:text-dark-text-secondary text-text-secondary">
                {skill.isOfficial && (
                  <>
                ...
                {skill.version && (
                  <>
                    <span className="px-1.5 py-0.5 rounded dark:bg-dark-surface-hover bg-surface-hover font-medium">
                      v{skill.version}
                    </span>
                    <span>·</span>
                  </>
      ...
      {activeTab === 'marketplace' && (
        isLoadingMarketplace ? (
          <div className="text-center py-12 text-sm dark:text-dark-text-secondary text-text-secondary">
          ...
          <>
            ...
            {filteredMarketplaceSkills.length === 0 ? (
              ...
              <>
                ...
                {marketplacePagination.pageCount > 1 && (
                  <div className="mt-4 flex items-center justify-between">
                    <button
                      type="button"
                      onClick={() => setMarketplacePagination((prev) => ({
                        ...prev,
                        page: Math.max(1, prev.page - 1),
                      }))}
                      disabled={marketplacePagination.page <= 1}
                      className="px-3 py-1.5 text-xs rounded-lg border dark:border-dark-border border-border dark:text-dark-text text-text-primary disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {i18nService.t('paginationPrevious')}
                    </button>
                    <div className="text-xs dark:text-dark-text-secondary text-text-secondary">
                      {marketplacePagination.page} / {marketplacePagination.pageCount}
                    </div>
                    <button
                      type="button"
                      onClick={() => setMarketplacePagination((prev) => ({
                      ...
                      disabled={marketplacePagination.page >= marketplacePagination.pageCount}
                      className="px-3 py-1.5 text-xs rounded-lg border dark:border-dark-border border-border dark:text-dark-text text-text-primary disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {i18nService.t('paginationNext')}
                    </button>
                  </div>
                )}
              </>
      ...
      {selectedMarketplaceSkill && createPortal(
        ...
        >
          <div
            className="w-full max-w-md mx-4 rounded-2xl dark:bg-dark-surface bg-surface border dark:border-dark-border border-border shadow-2xl p-6"
            onClick={(event) => event.stopPropagation()}
          >
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-9 h-9 rounded-lg dark:bg-dark-bg bg-page flex items-center justify-center flex-shrink-0">
                  <PuzzleIcon className="h-5 w-5 dark:text-dark-text-secondary text-text-secondary" />
                </div>
                <div className="min-w-0">
                  <div className="text-base font-semibold dark:text-dark-text text-text-primary truncate">
              ...
              <button
                type="button"
                onClick={() => setSelectedMarketplaceSkill(null)}
                className="p-1.5 rounded-lg dark:text-dark-text-secondary text-text-secondary dark:hover:text-dark-text hover:text-text-primary dark:hover:bg-dark-surface-hover hover:bg-surface-hover transition-colors flex-shrink-0"
              >
                <XMarkIcon className="h-5 w-5" />
              </button>
            </div>

            <p className="text-sm dark:text-dark-text-secondary text-text-secondary mb-4">
            ...
            <div className="space-y-2 mb-5">
              {selectedMarketplaceSkill.version && (
                <div className="flex items-center text-xs">
                  <span className="w-16 flex-shrink-0 dark:text-dark-text-secondary text-text-secondary">{i18nService.t('skillDetailVersion')}</span>
                  <span className="px-1.5 py-0.5 rounded dark:bg-dark-surface-hover bg-surface-hover dark:text-dark-text text-text-primary font-medium">
              ...
              {selectedMarketplaceSkill.source?.from && (
                <div className="flex items-center text-xs">
                  <span className="w-16 flex-shrink-0 dark:text-dark-text-secondary text-text-secondary">{i18nService.t('skillDetailSource')}</span>
                  <span className="px-1.5 py-0.5 rounded dark:bg-dark-surface-hover bg-surface-hover dark:text-dark-text text-text-primary font-medium">
                    {selectedMarketplaceSkill.source.from}
                  </span>
                  {selectedMarketplaceSkill.source.author && (
                    <span className="ml-1.5 px-1.5 py-0.5 rounded dark:bg-dark-surface-hover bg-surface-hover dark:text-dark-text text-text-primary font-medium">
                      {selectedMarketplaceSkill.source.author}
                    </span>
                  )}
                </div>
              ...
              {selectedMarketplaceSkill.source?.url && (
                <div className="flex items-start text-xs">
                  <span className="w-16 flex-shrink-0 dark:text-dark-text-secondary text-text-secondary pt-0.5">URL</span>
                  <button
                    type="button"
            ...
            ) : selectedMarketplaceSkill.permission?.canUse === false ? (
              <div className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-amber-500/10 text-amber-600 dark:text-amber-400 text-sm font-medium">
                {i18nService.t('skillNoAccess')}
              </div>
            ) : (
              <button
        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-between">
              ...
              <div
                className={`w-9 h-5 rounded-full flex items-center transition-colors cursor-pointer flex-shrink-0 ${
                  selectedSkill.enabled ? 'bg-primary' : 'dark:bg-dark-border bg-border'
                }`}
                onClick={() => {
      ...
      {skillPendingDelete && createPortal(
        ...
        >
          <div
            className="w-full max-w-sm mx-4 rounded-2xl dark:bg-dark-surface bg-surface border dark:border-dark-border border-border shadow-2xl p-5"
            onClick={(event) => event.stopPropagation()}
          >
            <div className="text-lg font-semibold dark:text-dark-text text-text-primary">
              {i18nService.t('deleteSkill')}
            </div>
            <p className="mt-2 text-sm dark:text-dark-text-secondary text-text-secondary">
            ...
            <div className="mt-4 flex items-center justify-end gap-2">
              <button
                ...
                className="px-3 py-1.5 text-xs rounded-lg border dark:border-dark-border border-border dark:text-dark-text-secondary text-text-secondary dark:hover:bg-dark-surface-hover hover:bg-surface-hover transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
              >
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="mt-1 text-sm dark:text-dark-text-secondary text-text-secondary">
              ...
              <button
                type="button"
                onClick={() => setIsGithubImportOpen(false)}
                className="p-1.5 rounded-lg dark:text-dark-text-secondary text-text-secondary dark:hover:text-dark-text hover:text-text-primary dark:hover:bg-dark-surface-hover hover:bg-surface-hover transition-colors"
              >
                <XMarkIcon className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="mt-5 space-y-3">
              ...
              <input
                ...
                className="w-full px-3 py-2.5 text-sm rounded-xl dark:bg-dark-bg bg-page dark:text-dark-text text-text-primary dark:placeholder-dark-text-secondary placeholder-text-secondary border dark:border-dark-border border-border focus:outline-none focus:ring-2 focus:ring-primary"
              />
