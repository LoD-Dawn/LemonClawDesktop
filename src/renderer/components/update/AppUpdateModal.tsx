...
const AppUpdateModal: React.FC<AppUpdateModalProps> = ({
  ...
  return (
    ...
    >
      <div
        className="modal-content w-full max-w-md mx-4 dark:bg-dark-surface bg-surface rounded-2xl shadow-modal overflow-hidden"
      ...
      >
        ...
        {modalState === 'info' && (
          <>
            <div className="px-5 pt-5 pb-4">
              ...
              {currentLog.title && (
                <p className="mt-3 text-sm font-medium dark:text-dark-text text-text-primary">
                  {currentLog.title}
                </p>
              )}
              ...
              {currentLog.content.length > 0 && (
                <ul className="mt-2 space-y-1.5 max-h-48 overflow-y-auto">
                  {currentLog.content.map((item, index) => (
                    <li key={index} className="flex items-start gap-2 text-sm dark:text-dark-text-secondary text-text-secondary">
            ...
            <div className="px-5 pb-5 flex items-center justify-end gap-2">
              <button
                type="button"
                onClick={onCancel}
                className="px-3 py-1.5 text-sm rounded-lg dark:text-dark-text-secondary text-text-secondary dark:hover:bg-dark-surface-hover hover:bg-surface-hover transition-colors"
              >
        ...
        {modalState === 'downloading' && (
          <div className="px-5 py-5">
            ...
            <div className="mt-4">
              ...
              {/* Progress info */}
              <div className="mt-2 flex items-center justify-between text-xs dark:text-dark-text-secondary text-text-secondary">
                <span>
                  {downloadProgress
        ...
        {modalState === 'installing' && (
          <div className="px-5 py-5">
            <div className="flex flex-col items-center py-4">
              ...
              <h3 className="mt-4 text-base font-semibold dark:text-dark-text text-text-primary">
                {i18nService.t('updateInstalling')}
              </h3>
              <p className="mt-1.5 text-xs dark:text-dark-text-secondary text-text-secondary text-center">
        ...
        {modalState === 'error' && (
          <div className="px-5 py-5">
            ...
            {errorMessage && (
              <p className="mt-2 text-sm dark:text-dark-text-secondary text-text-secondary break-words">
                {errorMessage}
              </p>
            )}
