// vendor/openclaw-runtime/win-x64/extensions/telegram/index.ts
import { emptyPluginConfigSchema } from "openclaw/plugin-sdk";

// vendor/openclaw-runtime/win-x64/extensions/telegram/src/channel.ts
import {
...
// vendor/openclaw-runtime/win-x64/extensions/telegram/src/runtime.ts
var runtime = null;
...
// vendor/openclaw-runtime/win-x64/extensions/telegram/src/channel.ts
var meta = getChatChannelMeta("telegram");
...
var telegramPlugin = {
  ...
  config: {
    ...
    resolveAllowFrom: ({ cfg, accountId }) => (resolveTelegramAccount({ cfg, accountId }).config.allowFrom ?? []).map(
      (entry) => String(entry)
    ),
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      const basePath = useAccountPath ? `channels.telegram.accounts.${resolvedAccountId}.` : "channels.telegram.";
      return {
  ...
  outbound: {
    ...
    sendPoll: async ({ to, poll, accountId, threadId, silent, isAnonymous }) => await getTelegramRuntime().channel.telegram.sendPollTelegram(to, poll, {
      accountId: accountId ?? void 0,
  ...
  status: {
    ...
    probeAccount: async ({ account, timeoutMs }) => getTelegramRuntime().channel.telegram.probeTelegram(
      account.token,
      timeoutMs,
    ...
    buildAccountSnapshot: ({ account, cfg, runtime: runtime2, probe, audit }) => {
      ...
      return {
        ...
        lastError: runtime2?.lastError ?? duplicateTokenReason,
        mode: runtime2?.mode ?? (account.config.webhookUrl ? "webhook" : "polling"),
        probe,
        audit,
  ...
  gateway: {
    ...
    logoutAccount: async ({ accountId, cfg }) => {
      ...
      if (nextTelegram) {
        ...
        const accounts = nextTelegram.accounts && typeof nextTelegram.accounts === "object" ? { ...nextTelegram.accounts } : void 0;
