// vendor/openclaw-runtime/win-x64/extensions/phone-control/index.ts
import fs from "fs/promises";
import path from "path";
var STATE_VERSION = 2;
var STATE_REL_PATH = ["plugins", "phone-control", "armed.json"];
var GROUP_COMMANDS = {
...
async function readArmState(statePath) {
  try {
    ...
    if (parsed.version === 1) {
      if (!Array.isArray(parsed.removedFromDeny) || !parsed.removedFromDeny.every((v) => typeof v === "string")) {
        return null;
      }
      return parsed;
    }
    ...
    if (!Array.isArray(parsed.armedCommands) || !parsed.armedCommands.every((v) => typeof v === "string")) {
      return null;
    }
    if (!Array.isArray(parsed.addedToAllow) || !parsed.addedToAllow.every((v) => typeof v === "string")) {
      return null;
    }
    if (!Array.isArray(parsed.removedFromDeny) || !parsed.removedFromDeny.every((v) => typeof v === "string")) {
      return null;
...
function formatStatus(state) {
  ...
  const cmds = uniqSorted(
    state.version === 1 ? state.removedFromDeny : state.armedCommands.length > 0 ? state.armedCommands : [...state.addedToAllow, ...state.removedFromDeny]
  );
