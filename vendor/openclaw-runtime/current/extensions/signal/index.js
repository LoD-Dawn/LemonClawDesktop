// vendor/openclaw-runtime/win-x64/extensions/signal/index.ts
import { emptyPluginConfigSchema } from "openclaw/plugin-sdk";

// vendor/openclaw-runtime/win-x64/extensions/signal/src/channel.ts
import {
...
// vendor/openclaw-runtime/win-x64/extensions/signal/src/runtime.ts
var runtime = null;
...
var meta = getChatChannelMeta("signal");
...
var signalPlugin = {
  ...
  config: {
    ...
    resolveAllowFrom: ({ cfg, accountId }) => (resolveSignalAccount({ cfg, accountId }).config.allowFrom ?? []).map(
      (entry) => String(entry)
    ),
    ...
    resolveDefaultTo: ({ cfg, accountId }) => resolveSignalAccount({ cfg, accountId }).config.defaultTo?.trim() || void 0
  },
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      const resolvedAccountId = accountId ?? account.accountId ?? DEFAULT_ACCOUNT_ID;
      const useAccountPath = Boolean(cfg.channels?.signal?.accounts?.[resolvedAccountId]);
      const basePath = useAccountPath ? `channels.signal.accounts.${resolvedAccountId}.` : "channels.signal.";
      return {
