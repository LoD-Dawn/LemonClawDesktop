// vendor/openclaw-runtime/win-x64/extensions/device-pair/index.ts
import os from "os";
import {
...
async function resolveTailnetHost() {
  return await resolveTailnetHostWithRunner(
    (argv, opts) => runPluginCommandWithTimeout({
      argv,
...
function resolveRequiredAuth(mode, values) {
  if (mode === "token") {
    return values.token ? { token: values.token, label: "token" } : { error: "Gateway auth is set to token, but no token is configured." };
  }
  return values.password ? { password: values.password, label: "password" } : { error: "Gateway auth is set to password, but no password is configured." };
}
    ...
    handler: async (ctx) => {
      ...
      if (action === "approve") {
        ...
        if (requested) {
          ...
          return {
            text: `${formatPendingRequests(list.pending)}
...
/pair approve latest`
      ...
      if (action === "qr") {
        ...
        const authLabel2 = auth.label ?? "auth";
        const channel2 = ctx.channel;
        const target2 = ctx.senderId?.trim() || ctx.from?.trim() || ctx.to?.trim() || "";
        if (channel2 === "telegram" && target2) {
          try {
        ...
        api.logger.info?.(`device-pair: QR fallback channel=${channel2} target=${target2}`);
        const infoLines = [
          `Gateway: ${payload.url}`,
          `Auth: ${authLabel2}`,
          "",
